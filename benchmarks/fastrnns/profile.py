...
def describe_sizes(**sizes):
    ...
    return f"s{sizes['seqLength']}-l{sizes['numLayers']}-i{sizes['inputSize']}-h{sizes['hiddenSize']}-b{sizes['miniBatch']}"
...
def full_profile(rnns, **args):
    profile_args = [f'--{k}={v}' for k, v in args.items()]
    profile_args.extend((f"--rnns {' '.join(rnns)}", '--internal-run'))
